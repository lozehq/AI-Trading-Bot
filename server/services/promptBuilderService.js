// 提示词构建服务：集中管理长模板与片段组合

class PromptBuilderService {
  /**
   * 构建AI分析提示词
   * @param {Object} args
   * @param {Object} args.marketData
   * @param {Object} args.indicators
   * @param {Object} args.newsData
   * @param {string} args.historyContext
   * @param {Object} args.dataQuality
   * @param {Object} args.anomalies
   * @param {Object} args.accountState - 账户状态信息（余额、持仓、交易绩效等）
   * @param {Object} args.formatters - 注入格式化函数，避免重复实现
   */
  buildAnalysisPrompt({ marketData, indicators, newsData, historyContext, dataQuality, anomalies, accountState, formatters, mode = 'complete' }) {
    ...
    const sections = [
      F.buildHeader(),
      '',
      F.buildDataQualitySection(dataQuality),
      '',
      F.buildAnomaliesSection(anomalies),
      '',
      // 添加账户状态监控section（核心决策依据）
      accountState ? F.buildAccountStateSection(accountState) : null,
      accountState ? '' : null,
      F.buildCurrentPriceSection(marketData),
      '',
      isMinimal ? null : F.buildPriceActionSection(formatters, marketData.priceAction),
      '',
      isMinimal ? null : F.buildMultiTimeframeSection(formatters, multiTimeframe),
      '',
      F.buildTechnicalIndicatorsSection(formatters, indicators),
      '',
      isFast || isMinimal ? null : F.buildDivergenceSection(formatters, marketData),
      '',
      F.buildNewsSection(formatters, newsData),
      '',
      isMinimal ? null : F.buildDerivativesSection(formatters, marketData),
      '',
      isFast || isMinimal ? null : F.buildOnChainSection(formatters, marketData),
      '',
      isFast || isMinimal ? null : F.buildAdvancedIndicatorsSection(formatters, marketData),
      '',
      isFast || isMinimal ? null : F.buildSocialDevSection(formatters, marketData),
      '',
      isMinimal ? null : F.buildOrderBookSection(formatters, marketData),
      '',
      isMinimal ? null : F.buildTradesSection(formatters, marketData),
      '',
      F.buildOHLCVSection(formatters, marketData),
      '',
      isFast || isMinimal ? null : F.buildTailContexts(marketData.mcpContext, historyContext),
      '',
      this.getRulesSection(marketData, indicators, mode)
    ];
  ...
  // 拆出规则大段，便于维护/复用/国际化
  getRulesSection(marketData, indicators, mode = 'complete') {
    if (mode === 'narrative') {
      return `
## 🎯 叙述模式（自我行为日志）
...
- 当指标分歧较大（>=2项核心冲突）时，倾向HOLD`;
    ...
    const __aggr = runtimeStrategy.getAggressivenessParams ? runtimeStrategy.getAggressivenessParams() : null;
    const __minConf = (__aggr && Number(__aggr.riskControlMinConfidence)) || 70;
    const __posBand = __aggr
      ? (__aggr.tradingRiskPercentage >= 3.0
          ? '25-35%'
          : (__aggr.tradingRiskPercentage >= 2.5 ? '20-30%' : '15-25%'))
      : '20-30%';
    return `
## 🎯 分析要求
...
- 如果上次信号是SELL，改为BUY建议：价格突破止损位 或 至少3个核心指标转多
 - 如果上次信号是HOLD，改为BUY/SELL建议：置信度≥${__minConf + 5}% 且 有明确突破信号

**2. 置信度要求（动态）**：
- BUY/SELL信号：置信度建议≥${__minConf}%（随激进度调整），否则建议给HOLD
- HOLD信号：当指标矛盾、趋势不明时使用
- 低置信度（<${__minConf}%）时，建议观望不要盲目开单
